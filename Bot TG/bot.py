...
def extract_event_details(text_or_prompt: str, timezone_offset: int = 4) -> dict:
    ...
    prompt = f"""
Ты — умный планировщик задач и ассистент с доступом к Gmail и Google Календарю через n8n.
...
1. Проверь, просит ли пользователь отправить письмо по почте (Gmail) или найти/показать почту.
   Если да, верни action "send_email" или "read_email".
2. Если это создание события или напоминания:
   - Вычисли время самого СОБЫТИЯ (`event_datetime` в ISO формате YYYY-MM-DDTHH:MM:SS) по часовому поясу пользователя.
   - Если указано только время (например "в 17:40" или "на 10") и это время позже текущего ({user_now.strftime('%H:%M')}), считай событие СЕГОДНЯ {user_now.strftime('%Y-%m-%d')}.
...
{{
    "is_event": true/false,
    "is_email": true/false,
    "email_action": "send" или "read" или null,
    "email_to": "email или имя контакта" или null,
...
If not an event or email, return {{"is_event": false, "is_email": false}}.
"""
    ...
    for model_name in MODELS_FALLBACK:
        try:
            ...
            if res and res.text:
                ...
                if parsed.get("is_event") or parsed.get("is_email"):
                    return parsed
        except Exception as e:
            continue
    return {"is_event": False, "is_email": False}
...
async def process_event_or_chat(message: types.Message, raw_text: str):
    ...
    if extracted.get("is_email"):
        email_action = extracted.get("email_action", "send")
        to_address = extracted.get("email_to") or "получатель"
        subject = extracted.get("email_subject") or "Без темы"
        body_text = extracted.get("email_body") or raw_text
        
        reply_email = (
            f"✉️ **Команда для Gmail передана в n8n!**\n\n"
            f"👤 **Кому/Контакт:** {to_address}\n"
            f"📌 **Тема:** {subject}\n"
            f"💬 **Текст:** {body_text}\n\n"
            f"Воркфлоу n8n автоматически обработает отправку через ваш подключенный аккаунт Gmail!"
        )
        await send_reply(message, reply_email, parse_mode="Markdown")
        return
